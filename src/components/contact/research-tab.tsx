    ...
    <div>
      ...
      <div className="flex items-center gap-3 mb-4">
        <input
          ...
          className="flex-1 text-[12px] px-3 py-1.5 border border-zinc-200 rounded-lg focus:outline-none focus:border-zinc-400"
        />
      ...
      {Object.entries(sections).map(([sectionName, fields]) => {
        ...
        return (
          <div key={sectionName} className="mb-5">
            <h3 className="text-[9px] font-semibold tracking-widest uppercase text-zinc-400 pb-1.5 mb-2 border-b border-zinc-100">
              {sectionName}
            </h3>
              ...
              {filteredFields.map((fieldName) => {
                ...
                return (
                  <div
                    ...
                    className="border border-zinc-200/60 rounded-lg p-2.5 cursor-pointer hover:border-zinc-300 transition-colors"
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-[9px] font-medium text-zinc-400 tracking-wide uppercase">{fieldName}</span>
                      {fieldData?.source && (
                    ...
                    {isEditing ? (
                      <div className="mt-1">
                        <input
                          ...
                          className="text-[12px] w-full px-1.5 py-0.5 border border-zinc-200 rounded focus:outline-none focus:border-zinc-400"
                          onClick={(e) => e.stopPropagation()}
                        />
                        <div className="flex gap-1 mt-1">
                          <button
                            onClick={(e) => { e.stopPropagation(); saveField(sectionName, fieldName); }}
                            className="text-[10px] px-2 py-0.5 bg-zinc-900 text-white rounded"
                          >
                            Save
                          </button>
                          <button
                      ...
                      <p className={`text-[12px] mt-1 leading-snug ${fieldData?.field_value ? 'font-medium text-zinc-900' : 'text-zinc-300'}`}>
