...
export function CreateContactModal({ isOpen, onClose }: Props) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // Success — clear form, close, refresh server data
      setFirstName('');
      setLastName('');
      setCompanyName('');
      setEmail('');
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-xl w-[420px] p-5"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between mb-4">
          <span className="text-[15px] font-medium text-zinc-900">New Contact</span>
          <button
            onClick={onClose}
        ...
        <FieldBlock label="COMPANY NAME" value={companyName} onChange={setCompanyName} />
